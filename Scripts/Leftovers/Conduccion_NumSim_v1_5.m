...
%___________________________________________________________________________
    ...
    case 'e'
        phiIC = (Q_ic) / ((dz+dz_ic)*dy_ic*dx_ic);                             % Volumetric dissipation [W/m^3]
        %%% Define 2D mesh %%%
...
%         hDT=ones(N,Mx+1,My+1);
        ...
        Lx = dx;
Ly = dy;
        N = 5000;  %Discretizacion temporal
        M = 30;    %Discretizacion espacial
        Mx = M; My = 30;
        Dx = Lx/Mx; Dy = Ly/My;
        tc=3000;
        M = 1e5; Dt = tc/N;
        
        y = linspace(0,100,My+1);
        %%%%
        d_FR4 = 1.5e-3 -2*50e-6; %Revisar
d_Cu = 1.1*50e-6; 
d_IC = 3e-3; 
Lz = d_FR4+d_Cu;
Lz_IC = Lz + d_IC;
        C_IC = 20; %capacidad térmica (J/K)
        c_Cu = 385; %capacidad térmica (J/(kg*K))
                rho_Cu = 8960; %en kg/m^3
        rhoc_Cu = rho_Cu*c_Cu; %en J/(m^3*K)
        rhoc_FR4 = rho_FR4*c_FR4;
        c_eff = (rhoc_FR4*d_FR4 +rhoc_Cu*d_Cu) /(rho_Cu*d_Cu + rho_FR4*d_FR4);
        rho_eff = (rho_Cu*d_Cu + rho_FR4*d_FR4) / Lz;
        rhoc_eff = rho_eff * c_eff;
        a_eff = k_eff / rhoc_eff;

        vol_IC = 0.04*0.02*d_IC;
        rhoc_IC = C_IC / vol_IC; %en J/(m^3*K)
        rhoc_eff_IC = (rhoc_IC*d_IC + rhoc_FR4*d_FR4 +rhoc_Cu*d_Cu) / Lz_IC;
        a_eff_IC = k_eff_ic/rhoc_eff_IC;
        Lz = dz;
        Lz_IC = dz+dz_ic;

        %%%%
        eps = 1e-6;
        x_raw= [0, 0.02-eps, 0.02+eps, 0.04-eps, 0.04+eps, 0.06-eps, 0.06+eps, 0.08-eps, ...
            0.08+eps, 0.1-eps, 0.1+eps, 0.12-eps, 0.12+eps, 0.14];
        phi_raw = [0, 0, 5, 5, 0, 0, 5, 5, 0, 0, 5, 5, 0, 0] / (((dz+dz_ic)*dy_ic*dx_ic));
        k_raw = [k_eff, k_eff, k_eff_ic, k_eff_ic, k_eff, k_eff, k_eff_ic, k_eff_ic, ...
            k_eff, k_eff, k_eff_ic, k_eff_ic, k_eff, k_eff];
        rhoc_raw = [rhoc_eff, rhoc_eff, rhoc_eff_IC, rhoc_eff_IC, rhoc_eff, rhoc_eff, ...
            rhoc_eff_IC, rhoc_eff_IC, rhoc_eff, rhoc_eff, ...
            rhoc_eff_IC, rhoc_eff_IC, rhoc_eff, rhoc_eff];
        d_raw = [Lz, Lz, Lz_IC, Lz_IC, Lz, Lz, Lz_IC, Lz_IC, ...
            Lz, Lz, Lz_IC, Lz_IC, Lz, Lz];
        
        x = linspace(0,Lx,M+1);
        phi = interp1(x_raw, phi_raw, x);
        k = interp1(x_raw, k_raw, x);
        d = interp1(x_raw, d_raw, x);
        rhoc = interp1(x_raw, rhoc_raw, x);
        A = Ly*d;
...
%         for i = 1:Mx
%             for k = 1:My
%                 Fo_vectx(i,k)=a_eff_IC*dt/dx^2;   % Fourier's number
...
disp(['Stability requires Fo<0.5. It actually is = ',num2str(Fo)])
if (Fo > 1/2) 
disp('This is unstable; increase number of time steps') 
end
        ...
        for j = 2:N              %
            for i = 2:Mx
                for k = 2:My
                    T(j,i,k)= T(j-1,i,k) + ((Dt / ((rhoc2d(i,k))*z(i,k)))) * ...
