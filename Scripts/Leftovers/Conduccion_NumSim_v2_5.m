...
%___________________________________________________________________________
    ...
    case 'e'
        ...
        N = 5e4;                % # of time steps                                 % 1.9e5 works
        tsim = 900;             % Total simulation time [s]                         % 750 works
        %%% Initialise %%%
...
%                 z(i, j) = dz;
%                 y = Dy*j;
%                 if y>=0.03+1e-8 && y<=0.07
%                     if i>=limx(1)+1 && i<=limx(2)   %%%ED
%                         k_effxy(i, j) =  k_eff_ic;
% %                         ro_c(i, j) = ro_c_IC;
%                         V2d(i,j) = dx * (dz+dz_ic) * dy;
%                         C(i,j) = C_eff_ic;
%                         phi2d(i, j) = phi;%5/(0.02*0.04*0.0045);
%                         z(i, j) = dz+dz_ic;
%                     end
%                     if i>=(limx(3)+1) && i<=limx(4)
%                         k_effxy(i, j) =  k_eff_ic;
% %                         ro_c(i, j) = ro_c_IC;
%                         V2d(i,j) = dx * (dz+dz_ic) * dy;
%                         C(i,j) = C_eff_ic;
