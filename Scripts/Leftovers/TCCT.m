...
%___________________________________________________________________________
    ...
    case 'e'
        phi2DIC = (Q_ic) / ((dz+dz_ic)*dy_ic*dx_ic);                             % Volumetric dissipation [W/m^3]
        %%% Define 2D mesh %%%
        m =4e0;                 % Spatial Subdivisions                              % 5e0 works
        ...
        N = 2.5e5;              % # of time steps                                   % 3e5 works
        tsim = 3000;            % Total simulation time [s]                         % 2800 works
        %%% Initialise %%%
        ...
        phi_raw = [0, 0, phi2DIC, phi2DIC, 0, 0, phi2DIC, phi2DIC, 0, 0, phi2DIC, phi2DIC, 0, 0]; %/ ((dz+dz_ic)*dy*dx_ic);
        k_raw = [k_eff, k_eff, k_eff_ic, k_eff_ic, k_eff, k_eff, k_eff_ic, k_eff_ic, ...
                 k_eff, k_eff, k_eff_ic, k_eff_ic, k_eff, k_eff];
        C_raw = [C_eff, C_eff, C_eff_ic, C_eff_ic, C_eff, C_eff, ...
                    C_eff_ic, C_eff_ic, C_eff, C_eff, ...
                    C_eff_ic, C_eff_ic, C_eff, C_eff];
        
        d_raw = [dz, dz, (dz+dz_ic), (dz+dz_ic), dz, dz, (dz+dz_ic), (dz+dz_ic), ...
                 dz, dz, (dz+dz_ic), (dz+dz_ic), dz, dz];

        x = linspace(0,dx,Mx+1); 
        phi = interp1(x_raw, phi_raw, x);
        k1d = interp1(x_raw, k_raw, x);
        d = interp1(x_raw, d_raw, x);
        Cx = interp1(x_raw, C_raw, x);   
        ...
        vol_IC = 0.04*0.02*3e-3;
    for j = 1:My+1
        if y(j) > 30 && y(j) < 70   %Dentro del IC
            phi2d(:,j) =  phi2DIC;%phi(:)*((dz+dz_ic)*dy*dx_ic)/vol_IC;
            k_effxy(:,j) = k1d(:);
            C(:,j) = Cx(:);
            z(:,j) = d(:);
        else
             phi2d(:,j) = 0;       %Fuera del IC
             k_effxy(:,j) = k_eff;
             C(:,j) = C_eff;
             z(:,j) = dz;
        end
...
%         T_stat_plot2( ( ( (Mx/2)+1):Mx), 1:(My/2)) =   T_stat_plot2( ( ( (Mx/2)):-1:1), 1:(My/2));
%         T_stat_plot2( ( 1:(Mx)),((My/2)+1):My) =   T_stat_plot2( ( 1:(Mx)),((My/2)):-1:1);
%         T_stat_plot2(:,My+1) = T_stat_plot2(:,My);
%         max(max(T_stat_plot2));
%         % Contour plot
%         figure()
%         contourf(X,Y,T_stat_plot2')
%         % Surf plot
%         figure()
